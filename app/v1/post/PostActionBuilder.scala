  ...
  override def invokeBlock[A](request: Request[A],
    ...
    future.map { result =>
      request.method match {
        case GET | HEAD =>
          result.withHeaders("Cache-Control" -> s"max-age: 0")
          // since someone may trigger VDM scrapper at any time, make sure that the browser will not read from the cache
        case other =>
          result
      }
    }
